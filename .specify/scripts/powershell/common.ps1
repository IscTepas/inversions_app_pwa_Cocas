...
function Test-FeatureBranch {
    ...
    $isTeamBranch = ($raw -match '^team\d{2}/') -and (-not $hasMalformedTimestamp)
    if (-not $isSequential -and $Branch -notmatch '^\d{8}-\d{6}-' -and -not $isTeamBranch) {
        [Console]::Error.WriteLine("ERROR: Not on a feature branch. Current branch: $raw")
        [Console]::Error.WriteLine("Feature branches should be named like: 001-feature-name, 1234-feature-name, 20260319-143022-feature-name, or teamXX/feature-name")
        return $false
    }
    return $true
}
