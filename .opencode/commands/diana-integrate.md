...
| run_only | specify | clarify | plan | tasks | implement | refine | design | breakdown | build | discovery | architecture | decomposition | delivery | implement | (vacio) | Etapa unica a ejecutar cuando action="run" |
| from | specify | clarify | plan | tasks | implement | (vacio) | Etapa inicial para un pipeline parcial cuando action="all" |
...
3. Resumen de readiness por stage/engine
4. Lista de artefactos Diana que el engine debe consumir sin reinterpretar el canon
...
- Declarar orden de ejecucion recomendado y artefactos de entrada obligatorios.
- Declarar si el engine puede ejecutar implementacion directa o requiere reparto multi-equipo previo.
...
Verificar:
- Que el engine objetivo tenga mapeo valido en `sdd-engine-matrix.yaml`.
- Que el stage objetivo tenga required_skills resolubles.
- Que el handoff referencia artefactos canonicos vigentes.
- Que, si existe trabajo multi-equipo, el handoff apunte a `/diana.teams` antes de implementacion distribuida.
...
- Reportar cambios significativos frente a la version previa.
...
- `stage` puede usarse como alias de `run_only` cuando la intencion sea inequivoca.
...
- `until` define la ultima etapa a ejecutar.
...
- Si `from` es posterior a `until`, detener con error de configuracion.
...
**Regla**: Speckit lee los artefactos Diana en `teams/TEAM-XX/` como INPUT BASE canonico.
No los descarta ni los reinterpreta — los optimiza con su logica SDD.
...
3. `/diana.constitution action="generate"`
4. `/diana.teams action="topology"` — definir equipos y scope_primario
5. `/diana.skills action="validate"`
6. `/diana.knowledge`
7. `/diana.specify` -> `/diana.plan` -> `/diana.tasks` (genera por equipo en teams/TEAM-XX/)
8. `/diana.teams action="generate"` — roster + allocation + bootstrap
9. `/diana.integrate action="generate" engine="speckit" stage="implement"`
10. Por cada equipo: `speckit.specify` -> `speckit.plan` -> `speckit.tasks` -> `speckit.implement`
